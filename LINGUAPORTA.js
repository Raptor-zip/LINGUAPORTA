...
chrome.storage.local.get(null, function (data) {
    ...
    if (document.body.className == "page-problem") {
        question_type = document.getElementsByClassName("page-title")[0].children[0].innerText.split(")")[1];
    ...
    const exists = checkIfClassExists("login-pannel-title"); // クラスが存在するかどうかを確認
    console.log(exists); // 結果をコンソールに出力

    console.debug("document.body.className:" + document.body.className);
    if(document.getElementById("problem-area") != null){
    console.debug(document.getElementById("problem-area").className);
    }
    ...
    switch (site_name) {
        case "ログイン画面":
            ...
            */
            ...
            //ユーザーが設定を変更したとき、または、初めに表示されたときの処理
            ...
            document.getElementsByClassName("body")[0].innerHTML = "<form action='/user/seibido/' method='POST'><input type='hidden' name='login' value='login'><div class='login-error-block'></div><div class='input-with-icon login-input-text'><i class='las la-smile'></i><input type='text' name='id' autocomplete='username' placeholder='User ID'></div><div class='input-with-icon login-input-text'><i class='las la-key'></i><div class='password-wrapper'><input type='password' name='password' autocomplete='current-password' placeholder='Password'><button class='button button-trans passwdViewBtn' type='button'>表示</button></div></div><script>makePasswordVisible(document.querySelector('input[type=password]'));</script><div class='login-optx '><a href='resetpw.php'>パスワードを忘れた方&nbsp;<i class='las la-arrow-right'></i></a><a href='https://www.seibido.co.jp/linguaporta/register.html' target='_blank'>リンガポルタの使い方&nbsp;<i class='las la-arrow-right'></i></a></div><p>学習状況</p><table style='table-layout:fixed;'><tbody><tr><th>スコア</th><th>全期間順位</th><th>正答率</th><th>時間/問</th></tr><tr><td id='score'>--</td><td id='rank'>--</td><td id='rate'>--</td><td id='score_per_min'>--</td></tr></tbody></table><p title='全自動、半自動、OFF、スコア、遅延、正答率、単元を設定します'>設定</p><table style='table-layout:fixed; border:none;'><tbody><tr><td style='border:none; text-align:center;'><label><input type='radio' id='mode_selection_0' name='mode_selection' value='auto' title='全自動' checked=''>全自動</label></td><td style='border:none; text-align:center;'><label><input type='radio' id='mode_selection_1' name='mode_selection' value='assist' title='補助(半自動)'>補助(半自動)</label></td><td style='border:none; text-align:center;'><label><input type='radio' id='mode_selection_2' name='mode_selection' value='OFF' title='OFF'>OFF</label></td></tr></tbody></table><table id='table_setting'><tbody><tr id='tr_setting_score'><th style='width:7em;'>獲得するスコア</th><td id='current-value_score' style='width:3em; border-right:none;'></td><td><input type='range' id='slider_score' min='25' max='300' step='25' value='100' style='width:100%' title='自動実行で獲得するスコアを選択します'></td></tr><tr id='tr_setting_late'><th>回答入力遅延</th><td id='current-value_late' style='border-right:none;'></td><td><input type='range' id='slider_late' min='0' max='20' step='1' value='8' style='width:100%' title='回答入力時の遅延を選択します(管理者に自動化ツールと判断されないようにするため)'></td></tr><tr id='tr_setting_rate'><th>正答率</th><td id='current-value_correct_answer_rate' style='border-right:none;'></td><td><input type='range' id='slider_correct_answer_rate' min='50' max='125' step='1' value='115' style='width:100%' title='正答率を選択します(サーバー上に答えがない場合は50%になります)(管理者に自動化ツールと判断されないようにするため)'></td></tr><tr><th>単元</th><td colspan='2' style='text-align:left !important;'><label><input type='radio' id='unit_selection_0' name='unit_selection' value='word_meaning_selection_and_empty_replenishment' title='単語の意味と空所補充' checked=''>単語の意味と空所補充</label><label><input type='radio' id='unit_selection_1' name='unit_selection' value='word_meaning_selection' title='単語の意味のみ'>単語の意味のみ</label><label><input type='radio' id='unit_selection_2' name='unit_selection' value='empty_replenishment' title='空所補充のみ'>空所補充のみ</label></td></tr></tbody></table><div class='login-btn'><button type='submit' value='LOGIN' class='button button-secondary button-big'>スタート</button></div></form> <a href='https://github.com/Raptor-zip/LINGUAPORTA/tree/main' class='bookmark source'><div class='bookmark-info'><div class='bookmark-text'><div class='bookmark-title'>《使い方》【Android・PC対応】リンガポルタ自動化ツール</div></div><div class='bookmark-href'><img src='https://github.com/fluidicon.png' class='icon bookmark-icon'>https://github.com/Raptor-zip/LINGUAPORTA/tree/main</div></div></a>";
            ...
            if (data["configured_score"] != null) {
                document.getElementById("slider_score").value = Math.round(data["learning_time"] * 10 / data["score"]) / 10 + "秒";
            }
            if (data["configured_late"] != null) {
                document.getElementById("slider_late").value = Math.round(data["learning_time"] * 10 / data["score"]) / 10 + "秒";
            }
            if (data["configured_rate"] != null) {
                document.getElementById("slider_correct_answer_rate").value = Math.round(data["learning_time"] * 10 / data["score"]) / 10 + "秒";
            }
            //onchangeイベントを発火させて、ラベルを表示させる
            update_setting();

            add_node_list = ["slider_score","slider_late","slider_correct_answer_rate","unit_selection_0","unit_selection_1","unit_selection_2","mode_selection_0","mode_selection_1","mode_selection_2"]
            for(let i=0; i<add_node_list.length; i++){
                document.getElementById(add_node_list[i]).addEventListener('change', update_setting);
            }
        ...
        case "ホーム画面":
            ...
            organize_chrome_storage_local(); // なんでこのタイミングで呼び出すのか？
            if (data.set_score > -2) {
                chrome.storage.local.set({
                    "rank": 9999
                }, function () {
        ...
        case "空所補充-正解表示画面":
            ...
            answer_en = document.querySelector("#question_area > div.qu03 > input[type=text]").value.trim();
            if (key in data) {
                chrome.storage.local.set({
        ...
        case "単語の意味-問題出題画面":
            ...
            if (data.set_score > 0) {
                question_number = document.getElementById("question_td").innerText.split(")")[0].slice(1).split("-")[0];
                let unit_score = Number(document.getElementsByClassName("score-number")[1].innerText);
                console.log(data["wrong_question_queue"]);
                if (data["wrong_question_queue"] !== undefined) {
                if (data["wrong_question_queue"].includes(unit_score + 1) == true) {
                    //間違える
        ...
        case "単語の意味-正解表示画面":
            ...
            question_number = document.querySelector("#question_td > form:nth-child(1) > b").innerHTML.slice(5) - 1 + Number(document.querySelector("#content-study > table > tbody > tr:nth-child(2) > td > div.bloc-resp.bloc-resp-lessonname").innerText.split(")")[0].slice(1).split("-")[0]);
            key = "a" + String(question_number);
            answer_en = document.querySelector("#qu02").innerText;
            answer_jp = document.querySelector("#drill_form").innerText.slice(3);
            if (key in data) {
                chrome.storage.local.set({
        ...
        case "単語の意味-正解画面":
            ...
            question_number = document.getElementsByClassName("problem-title")[0].innerText.split("：")[1] - 1 + Number(document.getElementsByClassName("page-title")[0].children[0].innerText.split(")")[0].slice(1).split("-")[0]);
            key = "a" + String(question_number);
            chrome.runtime.sendMessage({
                "id": data.id,
                "request_type": "set",
        ...
        case "単語の意味-全問題終了画面":
            ...
            answer_en = document.querySelector("#qu02").innerText;
            answer_jp = document.querySelector("#drill_form").innerText.slice(4);
            question_number = document.querySelector("#question_td > form:nth-child(1) > b").innerHTML.slice(5) - 1 + Number(document.querySelector("#content-study > table > tbody > tr:nth-child(2) > td > div.bloc-resp.bloc-resp-lessonname").innerText.split(")")[0].slice(1).split("-")[0]);
            chrome.runtime.sendMessage({
                "id": data.id,
                "request_type": "set",
